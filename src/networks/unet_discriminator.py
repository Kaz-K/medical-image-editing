#from https://github.com/boschresearch/unetgan/blob/master/layers.py - with minor modifications
import numpy as np
import math
import functools
...
import networks.biggan as layers
...
def D_unet_arch(in_channels, ch=64, attention='64',ksize='333333', dilation='111111',out_channel_multiplier=1):
    arch = {}
    ...
    arch[512] = {'in_channels' :            [in_channels] + [ch*item for item in [1, 2, 4, 8, 8, 8, 16, 8*2, 8*2, 8*2, 4*2, 2*2, 1*2  , 1         ]],
                             'out_channels' : [item * ch for item in [1, 2, 4, 8, 8, 8, 16, 8, 8,   8,   4,   2,   1,   1          ]],
...
class Unet_Discriminator(nn.Module):
    ...
    def __init__(self, in_channels=9, D_ch=64, D_wide=True, resolution=128,
        ...
        if self.resolution==128:
            ...
            self.output_features = [7, 8, 9, 10, 11, 12, 13]

        self.out_channel_multiplier = 1#4
        # Architecture
        self.arch = D_unet_arch(in_channels, self.ch, self.attention , out_channel_multiplier = self.out_channel_multiplier  )[resolution]
    ...
    def forward(self, x, y=None):
        ...
        for index, blocklist in enumerate(self.blocks[:-1]):
            ...
            for block in blocklist:
                ...
                if index in self.output_features:
                    features_out.append(h)
        ...
        return out, bottleneck_out, features_out
